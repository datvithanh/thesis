...
Mô hình cơ sở được sử dụng trong đồ án \cite{heo2020clova} tuân theo hệ thống ba pha được mô tả như trong \ref{related-works}, đây là sự kết hợp giữa mô hình ResNet-34, lớp tổng hợp thống kê tập trung (Attentive Statistic Pooling - ASP) và hàm mất mát nguyên mẫu góc (Angular Prototypical - AP). Kiến trúc của mô hình được miêu tả trong Hình \ref{fig:ap-resnet}. 
...
Trong các mục tiếp theo, đồ án sẽ trình bày chi tiết thành phần của mô hình.
...
\subsection{Hàm mất mát nguyên mẫu góc} \label{ap-loss}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.7\textwidth]{images/overall-propose.png}
...
trong đó $\theta_{\bm{x}_{i, M}, \bm{c}_{k}}$ là góc giữa truy vấn $\bm{x}_{i, M}$ của người $i$ và nguyên mẫu $\bm{c}_{k}$ của người $k$, $w$ và $b$ là các trọng số học được, và $m$ là hệ số phạt lề. Thay vào công thức \ref{eq:nll-softmax}, ta được hàm mất mát AMP-cos như sau:
...
Phương pháp học chuyển tiếp được ứng dụng tương đối rộng rãi cho bài toán xác minh người nói dưới nhiều dạng khác nhau, chủ yếu để giải quyết vấn đề giữ liệu trong tác vụ mới. Trong các nghiên cứu \cite{nidadavolu2019low, xia2019cross}, các tác giả sử dụng phương pháp học đối kháng với mục đích thích ứng mô hình xác minh người nói trên dữ liệu thu bằng microphone sang dữ liệu điện thoại di động, mô hình huấn luyện trên dữ liệu chứa tiếng vang sang dữ liệu sạch và mô hình trên tiếng Anh sang tiếng Trung Quốc. Điểm chung của các tập dữ liệu mục tiêu là thời lượng ít hơn rất nhiều so với tập dữ liệu gốc và không có nhãn người nói. Trong lời giải đứng nhất trong cuộc thi VOXSRC-20 \cite{thienpondt2020cross}, tác giả Thienpondt sử dụng hàm mất mát HPM để thích ứng mô hình xác minh người nói tiếng Anh sang tiếng Ba Tư với bộ dữ liệu mục tiêu gồm 588 người nói.

Hiện nay, theo hiểu biết của tác giả thì chưa có nghiên cứu nào so sánh sự hiệu quả của việc sử dụng SGD thay cho Adam trong xác minh người nói. Nhiều nghiên cứu sử dụng SGD thay cho Adam trong việc huấn luyện các mạng lớn trong xử lý ngôn ngữ tự nhiên \cite{howard-ruder-2018-universal} hay thị giác máy tính \cite{he2016deep, mcilwaine2021jellynet}.

Việc nghiên cứu cách thêm hệ số phạt lề vào hàm mất mát phân loại nhằm tăng tính phân biệt tương đối phổ biến trong bài toán nhận dạng - xác minh mặt người. Để giải quyết điểm yếu biên quyết định yếu của softmax, các công trình \cite{liu2017sphereface, wang2018additive, deng2019arcface} đề xuất các hàm A-softmax, CosFace và ArcFace bằng cách thêm hệ số phạt lề theo các cách khác nhau. Hai hàm CosFace và ArcFace trở nên phổ biến cho bài toán xác minh người nói do cài đặt dễ dàng và hiệu năng cao \cite{chung2020defence, xiang2019margin}. 

Các hàm mất mát phân biệt dựa trên cặp hay bộ ba như RLL \cite{wang2019ranked}, LS \cite{oh2016deep} hay hàm triplet đều được thiết kế với hệ số phạt lề. Ngược lại, các hàm mất mát phân biệt vận hành trên một mini-batch nhiều câu nói như GE2E \cite{wan2018generalized}, prototypical \cite{snell2017prototypical} hay AP \cite{chung2020defence} đều không được thiết kế với hệ số phạt lề. Gần đây, tác giả Wei cùng cộng sự nghiên cứu đưa hệ số phạt lề vào hàm GE2E cho kết quả đáng mong đợi\cite{wei2020angular}.